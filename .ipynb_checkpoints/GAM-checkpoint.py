...
import time
from StructureEnsemble import prep_structures
...
class GAM:
    ...
    def run(self, structures, beads=None, frames=None, NPs=1):
        ...
        """
        ensemble = prep_structures(structures)
        
        # If not specified, use them all
        if beads is None:
            beads = np.arange(ensemble.Nbeads)
        if frames is None:
            frames = np.arange(ensemble.Nframes)
        
        # Preallocate memory for one structure frame and for the sectioning data
        structure = np.empty((ensemble.Nbeads, 3), dtype=np.float64)
